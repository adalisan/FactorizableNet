...
def proposal_target_layer(object_rois, gt_objects, gt_relationships, 
                gt_regions, n_classes_obj, voc_eos, is_training):
    ...
    if is_training:
        ...
        all_rois = np.vstack(
            (all_rois, np.hstack((zeros, gt_objects[:, :-1])))
        )
        ...
        object_labels, object_rois, bbox_targets, bbox_inside_weights, \
            phrase_labels, phrase_rois, region_labels, \
                mat_object, mat_phrase = _sample_rois(all_rois, \
        ...
        object_rois, phrase_rois, mat_object, mat_phrase  = _setup_connection(object_rois)
        object_labels, bbox_targets, bbox_inside_weights, bbox_outside_weights, phrase_labels, region_labels = [None] * 6
...
def _sample_rois(object_rois, gt_objects, gt_relationships, gt_regions, num_images, num_classes, voc_eos, is_training):
...
### prepare region targets
    region_labels = _sample_regions(gt_regions, num_images, voc_eos)
...
def _sample_regions(gt_regions, num_images, voc_eos):
    """Generate a random sample of RoIs comprising foreground and background
    examples.
    """
    # overlaps: (rois x gt_regions)
    rois_per_image = cfg.TRAIN.BATCH_SIZE_IMG / num_images
    gt_len = gt_regions.shape[0]
